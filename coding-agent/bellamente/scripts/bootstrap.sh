#!/usr/bin/env bash
# Bellamente bootstrap for any coding agent (Codex, Claude Code, Cursor, and similar):
# check platform, download, verify sha256, install the CLI, doctor, start.
# Run once on a fresh machine. There is no host supervisor, so this script
# also installs bella.sh to ~/.bellamente and starts the local server.
...
platform, or run Bellamente in a linux-x86_64 container and point your agent at it.
EOF
  exit 3
fi
...
# Install the CLI to the agent-independent runtime home so every agent calls one path.
cp "$HERE/bella.sh" "$DEST_DIR/bella.sh"
chmod +x "$DEST_DIR/bella.sh"

if [ -x "$BINARY" ] && curl -sf http://127.0.0.1:8080/health >/dev/null 2>&1; then
  echo "ALREADY_UP: Bellamente is healthy at 127.0.0.1:8080"
  echo "CLI installed at $DEST_DIR/bella.sh"
  exit 0
fi
...
bash "$DEST_DIR/bella.sh" start

cat <<EOF

NEXT_STEP (agent action): confirm health, then finish install.
  1. bash ~/.bellamente/bella.sh health   should return {"ok":true,...}
